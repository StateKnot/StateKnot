...
unconsumed pending-result 分页、完整 result-set 的 atomic successor barrier、append-only
consumption proof、未结算 invocation 的 exact-checkpoint advancement guard、精确 migration checksum/startup refusal，以及 PostgreSQL
16/17 的 v3 Attempt 回填、corruption、rollback、lost-ack、delayed retry、100 并发 journal
appender、24 并发 result/barrier checkpoint writer、24 并发 tool/model invocation writer 和 24 并发 pending-result writer 测试。Node-attempt
覆盖 durable-before-dispatch、lost-ack、跨 fence crash recovery、数据库时钟 delayed retry、
...
回滚同样在 PostgreSQL 16/17 验证。Core 已进一步冻结 integrity-bound interrupt
request/resolution 与 durable timer registration/firing：精确 journal 因果、schema-pinned
payload、action digest、principal/scope 子集授权、exclusive interrupt expiry、inclusive timer
due、closed schema、属性测试及 canonical fixture 均已验证。它仍不是完整 runtime。
interrupt/timer 的 PostgreSQL 原子持久化、协议专用 outbox dispatch adapter、自动
quarantine、跨租户公平与 recovery/dispatch loop、角色隔离、
...
fairness、interrupt/timer PostgreSQL 持久化、recovery/dispatch loop、协议 dispatch adapter，以及阶段 3 的其余运维与故障门禁
未完成，不能据此提前宣称阶段完成。
